...
use finance_together_api::{cbindings::{CEventHandlerFP, CUuid}, EventHandler};
use thiserror::Error;
use uuid::Uuid;
...
impl Loader {
    ...
    pub unsafe fn load_library(filename: &str) -> Result<()> {
        ...
        let main = unsafe { lib.get::<Symbol<unsafe extern "C" fn(CUuid)->CEventHandlerFP>>(b"pluginMain")?}; 
