...
impl<T> ErrorMapper<T> for Option<T> {
    #[inline(always)]
    #[expect(clippy::inline_always, clippy::print_stderr, reason = "config assertion wrapper. Should be inlined.")]
    fn error(self, error: ServiceError) -> Result<T, ServiceError> {
        ...
        if self.is_none() {
            ...
            let line = colorizer.blue(format!("[Debug]{error}: Option<{}> is None.", any::type_name::<T>()));
            eprintln!("{line}");
        }
        self.ok_or(error)
    }
...
impl ApplicationContext {
    ...
    /// 
    #[expect(clippy::too_many_arguments, reason = "needs all handler as arguments for construction.")]
    #[inline]
    pub fn new_unsafe(
